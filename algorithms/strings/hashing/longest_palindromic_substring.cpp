...
* time: O(nlogn)
* solution: Polynomial Hash + Binary Search
*/
...
void go() {
	...
	int n = s.size();
	PolynomialHash sHash(s), rHash(r);
	...
	for (int i = 1; i < n; i++) {
		...
		while (lo  <= hi ) {
			int mid = (lo + hi)/2;
			if ( sHash(i, mid) == rHash(n - i, mid) ) lo = mid + 1;
			else hi = mid - 1;
		}
		...
		while (lo <= hi) {
			int mid = (lo + hi)/2;
			if ( sHash(i + 1, mid) == rHash(n - i, mid) ) lo = mid + 1;
			else hi = mid - 1;
		}
